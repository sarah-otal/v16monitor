...
create_dgt_openlayers_map <- function(csv_path, filter_source = NULL, filter_cause = NULL) {
  ...
  if(!file.exists("mapa_base.html")) {
    h1 <- '<!DOCTYPE html><html><head><title>DGT Mapa de Balizas v16 Activas</title><link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/ol@v7.4.0/ol.css"><style>'
...
# Add this inside script.R instead of the old block:
#readme_path <- "README.md"
#if (file.exists(readme_path)) {
#  lines <- readLines(readme_path)
#  # This looks for the badge line or a specific placeholder and updates the time
#  # Or simply appends the time to the very end of the file
#  lines[length(lines) + 1] <- paste0("\n")
#  writeLines(lines, readme_path)


# --- ROBUST README UPDATE ---
readme_path <- "README.md"
history_path <- "../last_count.txt"

if (file.exists(readme_path)) {
  lines <- readLines(readme_path)

  # 1a. Get Madrid Time (UTC + 1)
  madrid_time <- format(Sys.time(), tz="Europe/Madrid", usetz=TRUE)
  
  # 1b. Identify current count (use your exact variable name here)
  n_beacons <- if(exists("df_map")) nrow(df_map) else 0 
  
  # 2. Handle Trend
  last_count <- if(file.exists(history_path)) as.numeric(readLines(history_path)) else n_beacons
  trend <- if(n_beacons > last_count) "📈" else if(n_beacons < last_count) "📉" else "➡️"
  writeLines(as.character(n_beacons), history_path)
  
  # 3. Create the new clean status line
  status_line <- paste0(" ### ⏱️Última Actualización: ", madrid_time, " | 🚨Número de Balizas Activas: ", n_beacons, " ", trend)
 
  # 4. Find the placeholder index
  idx <- which(grepl("", lines))
  
  if (length(idx) > 0) {
    # We construct a NEW object: everything before placeholder + placeholder + new line + everything after the OLD status line
    # This prevents the "stuttering" effect
    new_readme <- c(lines[1:idx], status_line, lines[(idx + 2):length(lines)])
    # Remove any NA values if the file was short
    new_readme <- new_readme[!is.na(new_readme)]
...
# --- AUTO-CLEANUP ON VM ---
  ...
  # Identify files older than 1 day (1440 minutes)
  now <- Sys.time()
  file_times <- file.info(all_csvs)$mtime
  old_files <- all_csvs[difftime(now, file_times, units = "mins") > 1440]
  
  # Delete them
  if(length(old_files) > 0) {
    file.remove(old_files)
    cat("Cleaned up", length(old_files), "old CSV files from VM storage.\n")
  }
}
